...
Mean is formally $\bar{x} = \frac{\Sigma^n_{i=1} x_i}{n}$ for $n$ samples, specifically using samples here and not population, hence $\bar{x}$. Dropping missing values
...
Variance, ie deviation from mean is $s^2 = \frac{\Sigma_{i=1}^n(x_i - \bar{x})^2}{n-1}$ aka *sum of squares*.
